...
A July 30 activity spot-check shows the same execution shape directly: one BTC
market printed separate 3.6, 10.0, and 42.0-share UP buys at $0.04, while one ETH
market printed 30.0, 40.9, 5.2, 18.8, and 5.2-share UP buys at $0.04. Earlier in
the same windows, 9-, 10-, and 11-share merges were interleaved with independently
sized BUY fills. These are consistent with partial, asynchronous executions and
incremental merging; they are not consistent with treating the activity feed as
a sequence of atomic, equal-size order pairs. Display-rounded values such as
`0.0 shares / $0.01` should not be treated as exact zero-quantity fills.
