...
pUSD balance → live market data → merge proof freshness. Prints PASS/FAIL per
...
async def main() -> int:
    ...
    try:
        ...
        report(bal >= cfg.capital.min_starting_pusd, "pUSD balance",
        ...
        try:
            ...
            report(bool(markets), "market discovery",
                   f"{len(markets)} windows found, {len(live)} accepting orders")
            if live:
                book = await client.get_order_book(token_id=live[0].up_token_id)
                report(bool(book.asks and book.bids), "order book read",
        ...
        if proof.exists():
            try:
                ...
                report(same_wallet, "merge proof",
